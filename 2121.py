...
def check_group_exists(group):
    ...
    try:
        subprocess.run(["getent", "group", group], check=True)
        return True
    except subprocess.CalledProcessError:
        return False
...
def main():
    ...
    # Check if user accounts and groups exist or create them if not
    if args.force_change or args.email_users:
        with open(args.E_FILE_PATH, 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                first_name = row['first_name']
                last_name = row['last_name']
                ...
                # Generate a unique username
                username = generate_username(first_name, last_name, set())
                ...
                if not check_user_exists(username):
                    ...
                    # Check for force password change option
                    if args.force_change and int(row.get('N', 1)) % 2 != 0:
                        force_password_change(username)

                    # Check for email option and send email
                    if args.email_users and email:
                        password = generate_password()
                        email_password(username, email, password)
